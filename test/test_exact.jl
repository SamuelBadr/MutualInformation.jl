...
@testset "Exact Method Tests" begin
    ...
    @testset "Product state (no entanglement)" begin
        ...
        f(x) = x == [1, 1] ? 1.0 : 0.0
        MI_matrix = MI.mutualinformation_exact(f, [2, 2])
    ...
    @testset "Three-qubit GHZ state" begin
        ...
        MI_matrix = MI.mutualinformation_exact(f, [2, 2, 2])
    ...
    @testset "Non-uniform local dimensions" begin
        ...
        # Product state
        f(x) = x == [1, 1] ? 1.0 : 0.0
        MI_matrix = MI.mutualinformation_exact(f, [2, 3])
    ...
    @testset "Single site (edge case)" begin
        ...
        f(x) = x == [1] ? 1.0 : 0.0
        MI_matrix = MI.mutualinformation_exact(f, [2])
    ...
    @testset "Four-qubit cluster state" begin
        ...
        MI_matrix = MI.mutualinformation_exact(f, [2, 2, 2, 2])
    ...
    @testset "QuanticsGrids integration" begin
        ...
        MI_matrix = MI.mutualinformation_exact(qf, fill(2, R))
    ...
    @testset "Partial trace helper functions" begin
        ...
        @test MI.index_to_config(1, [2, 2]) == [1, 1]
        @test MI.index_to_config(2, [2, 2]) == [2, 1]
        @test MI.index_to_config(3, [2, 2]) == [1, 2]
        @test MI.index_to_config(4, [2, 2]) == [2, 2]

        @test MI.config_to_index([1, 1], [2, 2]) == 1
        @test MI.config_to_index([2, 1], [2, 2]) == 2
        @test MI.config_to_index([1, 2], [2, 2]) == 3
        @test MI.config_to_index([2, 2], [2, 2]) == 4
    end
    ...
    @testset "Unified API - automatic method selection" begin
        ...
        MI_auto = MI.mutualinformation(f, 8)
        MI_explicit_exact = MI.mutualinformation_exact(f, fill(2, 8))
